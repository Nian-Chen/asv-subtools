...
class Wav2Vec2EncoderStableLayerNorm(nn.Module):
    ...
    def forward(
        ...
        for layer in self.layers:
            ...
            if skip_the_layer:
                # layer dropout for multi-gpu training in ASV-Subtools
                h, p = layer(
                    hidden_states,
                ...
                position_bias = position_bias + 0 * p
...
class AMSoftmaxLoss(nn.Module):
    ...
    def forward(self, hidden_states, labels, output_logits=True):
...
class Wav2Vec2ForXVector(Wav2Vec2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config.keys_to_ignore_at_inference = ["embeddings", "hidden_states", "attentions"]
        self.wav2vec2 = Wav2Vec2Model(config)
        num_layers = config.num_hidden_layers + 1  # transformer layers + input embeddings
        if config.use_weighted_layer_sum:
            self.layer_weights = nn.Parameter(torch.ones(num_layers) / num_layers)
        ...
        # self.objective = CrossEntropyLoss(reduction='sum')
    ...
    def forward(
        ...
        if labels is not None:
            ...
            loss, logits = self.objective(logits, labels, output_logits=True)
            # print(logits.shape)
        if not return_dict:
